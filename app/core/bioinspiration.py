...
def biomimetics_marketplace(query, eng_sim, bio_sim): 
    ...
    if "Product Idea" in categories:
        ...
        summary_agent = agent_text('product_kum_summarize')
        search_expert_text = 'Search KMUs for product development...'
        location_filter = ''
        yield {'type': 'progress', 'message': 'Brainstorming product ideas...'}
        query = llm.chat([ChatMessage(role="user", content=query),ChatMessage(role='system', content = enrich_agent)]).message.blocks[0].text
        yield {'type': 'progress', 'message': 'Ranking product ideas...'}
        ranked_ideas = llm.chat([ChatMessage(role='system', content = rank_agent), ChatMessage(role="user", content=query)]).message.blocks[0].text
        print('Ranked_ideas: ', ranked_ideas)
        #idea_lst = ranked_ideas[:-1].split('[')[1].split(',')
        idea_lst = ranked_ideas.split(',')
        print('Idea_lst: ', idea_lst)
        idea_dct_lst = []
        for idea in idea_lst: 
            yield {'type': 'progress', 'message': 'Searching for KMU active in '+ idea + '...'}
            print(idea)
            kmu = llm.chat([ChatMessage(role='system', content = kmu_agent), ChatMessage(role="user", content='IDEA:\n'+ idea + '\nQUERY:\n'+ initial_query)]).message.blocks[0].text
            print(kmu)
            yield {'type': 'progress', 'message': 'Describe the relevant business area of '+ kmu + '...'}
            attention_text = 'Attention!!! These are not necessarily real companies, as there is currently no solid data basis for determining this. '
            summary = attention_text + llm.chat([ChatMessage(role='system', content = summary_agent), ChatMessage(role="user", content='IDEA:\n'+ idea + '\nKMU:\n'+ kmu + '\nQUERY:\n' + initial_query)]).message.blocks[0].text
            idea_dct = {'author_name': idea, 'summary': summary, 'affiliations': kmu, 'sources': ['Please note that the company information and technology associations provided here are generated by a language model and are based on patterns in its training data, not on real-time or verified sources. The model may be incomplete, outdated, or inaccurate, and it may occasionally produce incorrect or speculative associations. These results should not be regarded as professional, commercial, or factual advice. Users should independently verify all company details and technical feasibility before making decisions.']}
            idea_dct_lst.append(idea_dct)
        dct = {'Query': initial_query, 
               'Location': location_filter, 
               'Authors': idea_dct_lst}
